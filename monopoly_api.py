...
_INTRO = (
    ...
    "⑧ 跨局不重复(自动·你啥都不用记):引擎按【两个玩家的名字+性别】自动认这对玩家——同一对玩家用同样的名字开局,就自动躲开你们玩过的任务和身份(优先发最久没出过的·尽量不重样)(改天接着玩也一样,不用记 token)。返回里的 history_note 会告诉你「这对之前玩过几局」——念给人类听;若他们说第一次玩却显示玩过=跟别人撞名了,让他们换个独特名字、或给个暗号(开局传 pair_code)就分开、原名能留。(player_token 只在删局/看局时用,去重不靠它)\n"
    "讲清楚、问好红线,就开始吧。"
)
